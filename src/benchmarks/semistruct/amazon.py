...
from src.tools.download_hf import download_hf_file
from src.tools.io import load_files, save_files
from src.tools.node import Node, df_row_to_dict, register_node
...
class AmazonSemiStruct(SemiStructureKB):
    ...
    def __init__(self, 
                 ...
                 **kwargs):
        """
        Initialize the AmazonSemiStruct class.
    ...
    def construct_raw_node_info(self, df_meta: pd.DataFrame, df_review: pd.DataFrame, df_qa: pd.DataFrame) -> dict:
        ...
        for name, df, columns in zip(['review', 'qa'], 
                                     [df_review, df_qa], 
                                     [review_columns, self.qa_columns]):
            for df_i in tqdm(df.itertuples()):
                asin = df_i.asin
                idx = self.asin2id[asin]
                node_info[idx][name].append(df_row_to_dict(df_i, columns))
